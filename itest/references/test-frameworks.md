...
|---|---|---|---|
| `build-tag` | Go | A build constraint restricts the file to a specific `go build`/`go test` invocation | Add `//go:build integration` as the first line of the file, before the `package` clause, followed by a blank line. The file only runs when the runner passes `-tags=integration` (e.g. `go test -tags=integration ./...`); confirm the repo's actual invocation and record it in `runner_commands` — a bare `go test ./...` silently skips the file. |
...
1. **CI workflow files** (`.github/workflows/*.yml`/`*.yaml`, `.gitlab-ci.yml`, `Jenkinsfile`,
