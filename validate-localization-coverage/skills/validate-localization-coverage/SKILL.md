...
```bash
uv run "${CLAUDE_PLUGIN_ROOT}/skills/validate-localization-coverage/scripts/check-i18n.py" -y
```

`${CLAUDE_PLUGIN_ROOT}/skills/validate-localization-coverage` is the absolute path to this skill's directory (provided by the harness at invocation). The script auto-discovers `.claude/i18n.config.json`.
